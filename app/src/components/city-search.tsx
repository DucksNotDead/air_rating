  ...
  (
    ...
    useEffect(() => {
      if (searchQuery?.length > 2) {
        fetch("http://localhost:8000/api/airports/?search=" + searchQuery).then(
          async (res) => {
            if (res.ok) {
    ...
    return (
      <div className={"relative flex-1"} style={{ zIndex: 3 }}>
        ...
        >
          {isOpen ? (
            responseItems.map(({ name, id }, index) => (
              <div
                data-id={id}
                key={id}
                className={
                  "cursor-pointer px-md py-sm" +
                  (index ? " border-t-[1px] border-lightGrey" : "")
                }
              >
                {name}
              </div>
